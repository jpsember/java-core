...
public class FindMedianFromDataStream extends LeetCode {
  ...
  class MedianFinder {
    ...
    private void shiftMedianLow() {
      ...
      checkState(midBucket.size() == BUCKET_SIZE_CENTRAL);
      ...
      System.arraycopy(lowBucket.array, lowBucket.size() - SHIFT_COUNT, newMidArray, 0, SHIFT_COUNT);
      System.arraycopy(midBucket.array, 0, newMidArray, SHIFT_COUNT, SHIFT_COUNT);

      // Remove them from the low bucket
      lowBucket.used -= SHIFT_COUNT;

      // Move the upper half of the central bucket items to the high bucket. They can be appended
      // to the end, as the high bucket doesn't need to be sorted

      highBucket.ensureCapacity(highBucket.size() + SHIFT_COUNT);
      System.arraycopy(midBucket.array, SHIFT_COUNT, highBucket.array, highBucket.size(), SHIFT_COUNT);
    ...
    private void shiftMedianHigh() {
      ...
      var newMidArray = new short[BUCKET_SIZE_CENTRAL];

      System.arraycopy(highBucket.array, 0, newMidArray, SHIFT_COUNT, SHIFT_COUNT);
      System.arraycopy(midBucket.array, SHIFT_COUNT, newMidArray, 0, SHIFT_COUNT);

      // Remove them from the high bucket
      System.arraycopy(highBucket.array, SHIFT_COUNT, highBucket.array, 0, highBucket.size() - SHIFT_COUNT);

      highBucket.used -= SHIFT_COUNT;

      // Move the lower half of the central bucket items to the low bucket.  They can go on the
      // end, as the bucket doesn't need to be sorted
      lowBucket.ensureCapacity(lowBucket.size() + SHIFT_COUNT);
      System.arraycopy(midBucket.array, 0, lowBucket.array, lowBucket.size(), SHIFT_COUNT);
      lowBucket.used += SHIFT_COUNT;
    ...
    public JSMap toJson() {
      ...
      m.put("1 low bucket", lowBucket.toJson());
      m.put("2 mid bucket", midBucket.toJson());
      m.put("3 high bucket", highBucket.toJson());
      return m;
    }
