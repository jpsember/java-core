  ...
  class MedianFinder {
    ...
    public double findMedian() {
     // db("findMedian", INDENT, this);
      var cleft = 0;
      int pleft = buck(cleft).size();
      var cright = buckets.size() - 1;
      int pright = buck(cright).size();
      while (true) {
        if (cleft == cright) {
          // Median lies in this bucket
          if (split(cleft))
            return findMedian();
          return buck(cleft).median();
          ...
          if (split(cleft)) {
            ...
            return findMedian();
          return (buck(cleft).maxVal() + buck(cright).minVal()) / 2.0;
        } else {
          var sub = Math.min(pleft, pright);

          pleft -= sub;
          pright -= sub;
          if (pleft == 0) {
            cleft++;
            pleft = buckets.get(cleft).size();
          } else {
            cright--;
            pright = buckets.get(cright).size();
          }
        }
      }
    }
