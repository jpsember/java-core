...
public class BestTimeToBuyAndSellStockIV extends LeetCode {
  ...
  public void run() {
    ...
    if (false) {
      ...
      //      xs(701+803+2 + 258 * 7823, 258,258,-1);
      //      if (true) return;
      //      
      for (int f1 = 0; f1 < 50; f1++) {
        for (int f2 = 3; f2 < 5000; f2++) {
          int seed = 0;
          int k = 0;
          int count = 0;

          try {
            seed = f1 * 7823 + f2 * 19823;
             k = 2 + f2 / 7;
            count = f2;
            pr("seed:", seed,"k:",k,"count:",count);
               xs(seed, k, count, -1);
          } catch (Throwable t) {
            if (minCount > count) {
              pr("best seed:", seed, "k:", k, "count:", count);
              minCount = count;
            }
            break;
          }
    ...
    if (true) {
      ...
      if (true) {
        x("[70,4,83,56,94,72,78,43,2,86,65,100,94,56,41,66,3,33,10,3,45,94,15,12,78,60,58,0,58,"
            ...
            + "16,62,89,36,74,9,49,62,89]", 29, -1);
        return;
      }
