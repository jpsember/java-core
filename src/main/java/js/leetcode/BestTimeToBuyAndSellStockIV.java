...
public class BestTimeToBuyAndSellStockIV extends LeetCode {
  ...
  public void run() {
    ...
    x("[527,92,902,746,778]", 3, -1); // Should produce 842!
    if (true) {
      for (int n = 5; n < 100; n++) {
        solveRandom(n * 17, 3, n, 1);
      }
      return;
    }
  ...
  private void dup(int times) {
    ...
    for (int i = 0; i < times; i++)
      workList.addAll(workList.subList(0, sz));
  }

  private void runWorkList(int k, int expected) {
    loadTools();
    int[] prices = DataUtil.intArray(workList);
    workList.clear();
  ...
  private void solveWithVerify(int[] prices, int k, int expected) {
    ...
    final boolean withTiming = (prices.length >= 100);
    pr(VERT_SP, "prices.length:", prices.length, "withTiming:", withTiming);

    if (expected < 0) {
      if (withTiming)
        checkpoint("Calculating SLOW max profit for n", prices.length);
